...
namespace Gorfector
{
    class Gorfector_JpegWriterTestsFixture : public testing::Test
    {
    protected:
        ...
        void SetUp() override
        {
            ...
            if (testInfo != nullptr)
            {
                fileName = std::string(testInfo->test_suite_name()) + "_" + std::string(testInfo->name()) + ".jpg";
            }
    ...
    TEST_F(Gorfector_JpegWriterTestsFixture, CanCreateJpegWriter)
    {
        ...
        JpegWriter writer(m_State, std::string(typeid(Gorfector_JpegWriterTestsFixture).name()));
    ...
    TEST_F(Gorfector_JpegWriterTestsFixture, CanWrite1bitJpeg)
    {
        SANE_Parameters saneParameters;
        ...
        JpegWriter writer(m_State, std::string(typeid(Gorfector_JpegWriterTestsFixture).name()));
        auto updater = JpegWriterState::Updater(writer.GetStateComponent());
        updater.SetQuality(100);
    ...
    TEST_F(Gorfector_JpegWriterTestsFixture, CanWrite8BitColorJpeg)
    {
        SANE_Parameters saneParameters;
        SANE_Byte *buffer = nullptr;
        size_t bufferSize = 0;
    ...
    TEST_F(Gorfector_JpegWriterTestsFixture, CanWrite8BitGrayscaleJpeg)
    {
        SANE_Parameters saneParameters;
        SANE_Byte *buffer = nullptr;
        size_t bufferSize = 0;
    ...
    TEST_F(Gorfector_JpegWriterTestsFixture, CanWrite16bitsColorJpeg)
    {
        SANE_Parameters saneParameters;
        SANE_Byte *buffer = nullptr;
        size_t bufferSize = 0;
    ...
    TEST_F(Gorfector_JpegWriterTestsFixture, CanWrite16bitsGrayscaleJpeg)
    {
        SANE_Parameters saneParameters;
        SANE_Byte *buffer = nullptr;
        size_t bufferSize = 0;
