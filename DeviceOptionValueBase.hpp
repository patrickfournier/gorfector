...
namespace ZooScan
{
    ...
    public:
        ...
        {
            auto caps = optionDescriptor.cap;
            bool notSettable = (caps & SANE_CAP_INACTIVE) ||
                               (caps & SANE_CAP_HARD_SELECT);

            if (notSettable)
                return true;

            bool hideBecauseOfTitle = optionDescriptor.title != nullptr &&
                                      (strcasestr(optionDescriptor.title, "deprecat") != nullptr ||
                                       strcasestr(optionDescriptor.title, "preview") != nullptr);

            if (hideBecauseOfTitle)
                return true;

            bool hideBecauseOfDesc = optionDescriptor.desc != nullptr &&
                                     (strcasestr(optionDescriptor.desc, "deprecat") != nullptr ||
